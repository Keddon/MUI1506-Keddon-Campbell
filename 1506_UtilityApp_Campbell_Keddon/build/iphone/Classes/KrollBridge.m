...
NSString * k1506_UtilityApp_Campbell_Keddon$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1506_UtilityApp_Campbell_KeddonObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1506_utilityapp_campbell_keddon);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1506_utilityapp_campbell_keddon gc];
}
...
{
	// create k1506_UtilityApp_Campbell_Keddon global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1506_UtilityApp_Campbell_Keddon" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1506_utilityapp_campbell_keddon = [[k1506_UtilityApp_Campbell_KeddonObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1506_utilityapp_campbell_keddon];
	
	NSString *_k1506_utilityapp_campbell_keddonNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1506_utilityapp_campbell_keddonNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1506_utilityapp_campbell_keddon valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1506_UtilityApp_Campbell_Keddon$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1506_utilityapp_campbell_keddon.kroll" 
